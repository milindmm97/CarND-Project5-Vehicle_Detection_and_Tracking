# -*- coding: utf-8 -*-
"""
Created on Wed Apr 18 17:44:49 2018
...
def main():
   ...
   # print('Total frames: {}'.format(clip.duration * clip.fps))
    cap = cv2.VideoCapture('F:/CarND-Project5-Vehicle_Detection_and_Tracking/Code/project_video.mp4')
    count=0
...
# Read until video is completed
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            print(type(dst))
            #count+1
            #result_frames.append(dst)
            #cv2.imshow('Frame',dst)
            '''
            if(count>30):
                
                cv2.imshow('Frame',result_frames[iter_])
                iter_+1
            '''    
